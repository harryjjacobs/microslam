...
void test_map_scan_score() {
  ...
  map_add_scan(&occupancy, &scan, &robot_pose);
...
void test_map_scan_match_gradient() {
  ...
  map_add_scan(&occupancy, &gt_scan, &robot_pose);

  pose_t gradient;
  float score;
  map_scan_match_gradient(&occupancy, &gt_scan, &robot_pose, &gradient, &score);
  TEST_ASSERT_FLOAT_WITHIN(1e-3, 0.0f, gradient.x);
