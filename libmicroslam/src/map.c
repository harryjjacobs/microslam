...
void map_scan_match_gradient(occupancy_quadtree_t *occupancy, scan_t *scan,
  ...
  const float inlier_distance = 0.3f;  // nearest points further than this are ignored
  ...
  *score = 0;
  for (unsigned short i = 0; i < 360; i += decimate) {
    ...
    if (leaf != NULL && distance < inlier_distance) {  // && distance > inlier_distance
      ...
      gradient->x += leaf->log_odds * distance * dd_dx;
      gradient->y += leaf->log_odds * distance * dd_dy;
      gradient->r += leaf->log_odds * distance * dd_dtheta;
  ...
  gradient->x /= (float)normalisation_factor;
  gradient->y /= (float)normalisation_factor;
  gradient->r /= (float)normalisation_factor;
...
unsigned char map_scan_match(occupancy_quadtree_t *occupancy, scan_t *scan,
  ...
  *score = -INFINITY;
  ...
  // gradient descent
  float step_size = 0.0002f;  // note - this depends on the log odds scale (because gradients are weighted by log odds)
  float current_score;
  pose_t current_estimate = prior->pose;
  ...
  while (i < iterations) {
    ...
    if (fabsf(current_estimate.x - last_update.x) < tolerance &&
        fabsf(current_estimate.y - last_update.y) < tolerance &&
        fabsf(current_estimate.r - last_update.r) < tolerance) {
      break;
    }
