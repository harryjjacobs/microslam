...
void move(robot_pose_t *state, odometry_t *motion) {
...
int main() {
  ...
  while (!glfwWindowShouldClose(viewer.window)) {
    ...
    odometry_t gt_motion;
    slam_odometry_reset(&gt_motion);
    gt_motion.covariance[0][0] = 0.1;  // mm
    gt_motion.covariance[1][1] = 0.1;  // mm
    gt_motion.covariance[2][2] = 0.05; // rad

    // get the key from the viewer
    ...
    case slam_viewer_key_up:
      ...
      gt_motion.dx = dt * linear_speed *
                     (1 + random_normalf(0, gt_motion.covariance[0][0])) *
                     cos(gt_state.pose.r);
      gt_motion.dy = dt * linear_speed *
                     (1 + random_normalf(0, gt_motion.covariance[1][1])) *
                     sin(gt_state.pose.r);
      break;
    ...
    case slam_viewer_key_left:
      ...
      gt_motion.dr = dt * angular_speed *
                     (1 + random_normalf(0, gt_motion.covariance[2][2]));
      break;
    ...
    if (key != slam_viewer_key_none) {
      ...
      if ((hypotf(state.pose.x - prev_update_pose.x,
        ...
        state.error.x += gt_motion.covariance[0][0] * update_distance;
        state.error.y += gt_motion.covariance[1][1] * update_distance;
        state.error.r += gt_motion.covariance[2][2] * update_angle;
